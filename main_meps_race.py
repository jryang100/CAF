...
meps_train = MEPS_RACE(pd.read_csv("datasets/meps_train.csv").values)
meps_test = MEPS_RACE(pd.read_csv("datasets/meps_test.csv").values)
num_classes = 2
num_groups = 2
...
if model == "mlp":
    ...
    np.savetxt('./meps_race_points/mlp.txt', data, header="acc, DP Gap, EO Gap, EOdds Gap", fmt='%.4f')
elif model == "caf":
    ...
    data = np.column_stack((overall_acc_list, dp_gap_list, equalized_odds_y0_list, equalized_odds_mean_list))
    np.savetxt('./meps_race_points/caf.txt', data, header="acc, DP Gap, EO Gap, EOdds Gap", fmt='%.4f')
else:
