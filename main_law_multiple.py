...
from dataset import LAW_MULTIPLE as LAW
...
law_train = LAW(lawdata[:num_train, :])
law_test = LAW(lawdata[num_train:, :])
...
race_idx = law_train.attrs_race == 0
sex_idx = law_train.attrs_sex == 0

# test data info
target_insts = torch.from_numpy(law_test.insts).float().to(device)
target_labels = law_test.labels
target_attrs_race = law_test.attrs_race
target_attrs_sex = law_test.attrs_sex
...
if model == "mlp":
    ...
    np.savetxt('./law_multiple_points/mlp_race.txt', race_data, header="acc, DP Gap, EO Gap, EOdds Gap", fmt='%.4f')
    sex_data = np.column_stack(
        (sex_overall_acc_list, sex_dp_gap_list, sex_equalized_odds_y0_list, sex_equalized_odds_mean_list))
    np.savetxt('./law_multiple_points/mlp_sex.txt', sex_data, header="acc, DP Gap, EO Gap, EOdds Gap", fmt='%.4f')
elif model == "caf":
    ...
    for reg_lmbda in [i * 0.01 for i in range(1, 1001, 10)]:
        ...
        for t in range(num_epochs):
            ...
            corr_sum = 0.0
            for xs, ys, attrs_race, attrs_sex in train_loader:
                xs, ys, attrs_race, attrs_sex = xs.to(device), ys.to(device), attrs_race.to(device), attrs_sex.to(
                    device)
                optimizer.zero_grad()
    ...
    race_data = np.column_stack(
        (race_overall_acc_list, race_dp_gap_list, race_equalized_odds_y0_list, race_equalized_odds_mean_list))
    np.savetxt('./law_multiple_points/caf_race.txt', race_data, header="acc, DP Gap, EO Gap, EOdds Gap", fmt='%.4f')
    sex_data = np.column_stack(
        (sex_overall_acc_list, sex_dp_gap_list, sex_equalized_odds_y0_list, sex_equalized_odds_mean_list))
    np.savetxt('./law_multiple_points/caf_sex.txt', sex_data, header="acc, DP Gap, EO Gap, EOdds Gap", fmt='%.4f')
else:
